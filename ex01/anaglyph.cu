...
__global__ void process(const cv::cuda::PtrStep<uchar3> src, cv::cuda::PtrStep<uchar3> dst, int rows, int cols) {
    ...
    dst(dst_y, dst_x/2).x = char(0.299*left.x + 0.587*left.y + 0.114*left.z);
    dst(dst_y, dst_x/2).y = 0;
    dst(dst_y, dst_x/2).z = char(0.299*right.x + 0.587*right.y + 0.114*right.z);
}
